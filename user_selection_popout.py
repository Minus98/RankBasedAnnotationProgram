...
class UserSelectionPopOut():
    ...
    def __init__(self, root, center, select_user_callback):
        ...
        self.saved_users_frame.grid(column = 0, row = 1, sticky = "new", padx = 20, columnspan = 2)
        ...
        self.user_entry = ctk.CTkEntry(self.pop_out, placeholder_text="Enter a new user" , width=200, height=45, font=('Helvetica bold', 18), textvariable=self.new_name_var)
        self.user_entry.grid(column = 0, row = 2, sticky = "ne", padx=(0,5))
        self.user_entry.bind("<Return>", lambda event: self.on_return())

        self.new_button = ctk.CTkButton(master = self.pop_out, text="+", width=45, height=45, font=('Helvetica bold', 20), command=self.new_user, state=ctk.DISABLED)
    ...
    def append_user_row(self, i, user):
        ...
        delete_button = ctk.CTkLabel(master=saved_users_row, text="X", text_color="#C00000", font=('Helvetica bold', 20))
        delete_button.grid(row=0, column=1, padx=10, pady=4, sticky="e")
        delete_button.bind("<Enter>", lambda event, widget=delete_button: widget.configure(text_color="#F00000"))
        delete_button.bind("<Leave>", lambda event, widget=delete_button: widget.configure(text_color="#C00000"))

        delete_button.bind("<Button-1>", lambda event, i = i: self.delete_user(i))
